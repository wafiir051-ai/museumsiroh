...
export default function RegisterPage() {
  ...
  useEffect(() => {
    supabase.from('admins').select('*', { count: 'exact', head: true }).then(({ count }) => {
  ...
  async function handleSubmit(e) {
    ...
    if (signUpError) {
      setLoading(false)
      setError(signUpError.message.includes('already registered') ? 'Email ini sudah terdaftar. Silakan masuk.' : signUpError.message)
      return
    }
    ...
    if (session && userId) {
      const { error: insertError } = await supabase.from('affiliates').insert({
        user_id: userId, full_name: form.fullName, email: form.email, phone: form.phone || null,
      })
      setLoading(false)
      if (insertError) { setError('Akun dibuat, tapi profil mitra gagal: ' + insertError.message); return }
      navigate('/menunggu-approval')
    } else {
      sessionStorage.setItem('siroh_pending', JSON.stringify({ full_name: form.fullName, phone: form.phone || null }))
      setLoading(false)
  ...
  if (waitingEmail) return (
    <div className="flex min-h-screen items-center justify-center bg-siroh-paper lattice-bg px-6 py-10 dark:bg-siroh-charcoal">
      <div className="card p-8 max-w-sm w-full text-center">
        <div className="text-5xl mb-4">📧</div>
        <h2 className="font-display text-2xl font-semibold mb-2">Cek email kamu</h2>
        <p className="text-sm text-siroh-ink/60 mb-6 dark:text-white/60">
          Link konfirmasi sudah dikirim ke <strong>{form.email}</strong>.<br />
          Klik link tersebut, lalu masuk untuk menyelesaikan pendaftaran.
        </p>
    ...
    <div className="flex min-h-screen items-center justify-center bg-siroh-paper lattice-bg px-6 py-10 dark:bg-siroh-charcoal">
      <div className="w-full max-w-sm">
        ...
        <div className="card p-8">
          <h1 className="font-display text-2xl font-semibold">Daftar jadi mitra</h1>
          <p className="mt-1 text-sm text-siroh-ink/60 dark:text-white/60">Gratis. Disetujui dalam 1–2 hari kerja.</p>
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input required value={form.fullName} onChange={update('fullName')} className="input-field pl-11" placeholder="Nama sesuai KTP" />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" required value={form.email} onChange={update('email')} className="input-field pl-11" placeholder="nama@email.com" />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input value={form.phone} onChange={update('phone')} className="input-field pl-11" placeholder="08xxxxxxxxxx" />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" required value={form.password} onChange={update('password')} className="input-field pl-11" placeholder="Minimal 6 karakter" />
              </div>
            </div>
            {error && (
            ...
            <button type="submit" disabled={loading} className="btn-gold w-full disabled:opacity-60">
              {loading ? 'Memproses…' : 'Daftar Sekarang'} <ArrowRight className="h-4 w-4" />
            </button>
          </form>
          ...
          {!hasAdmin && (
            <p className="mt-2 text-center text-sm text-siroh-ink/40 dark:text-white/40">
              Admin belum ada?{' '}
              <RouterLink to="/admin/setup" className="font-semibold text-siroh-ink/50 hover:underline dark:text-white/50">
                Setup admin
              </RouterLink>
            </p>
          )}
